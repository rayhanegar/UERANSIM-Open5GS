...
# Port forwarding for external gNB access via Tailscale tunnel
# Get Tailscale tunnel IP from tailscale0 interface
TAILSCALE_IP=$(ip -4 addr show tailscale0 2>/dev/null | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | head -n1)

if [ -z "$TAILSCALE_IP" ]; then
    echo "Warning: Tailscale interface (tailscale0) not found or no IP assigned"
    echo "Please ensure Tailscale is running and authenticated"
    exit 1
fi
...
echo "4. Your gNB should connect to $TAILSCALE_IP:38412 for N2 interface"
echo "5. Your gNB should connect to $TAILSCALE_IP:2152 for N3 interface"
echo ""
