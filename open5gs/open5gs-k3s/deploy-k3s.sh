...
if ! kubectl wait --for=condition=ready pod -l app=nrf -n open5gs --timeout=120s; then
...
if ! kubectl wait --for=condition=ready pod -l app=scp -n open5gs --timeout=120s; then
...
if ! kubectl wait --for=condition=ready pod -l component=control-plane -n open5gs --timeout=180s; then
...
if ! kubectl wait --for=condition=ready pod -l app=amf -n open5gs --timeout=120s; then
...
if ! kubectl wait --for=condition=ready pod -l app=smf -n open5gs --timeout=120s; then
...
if ! kubectl wait --for=condition=ready pod -l app=upf -n open5gs --timeout=120s; then
